...
const Auth = ({ onAuthSuccess }: AuthProps) => {
  ...
  const handleGoogleAuth = async () => {
    ...
    try {
      ...
      if (typeof error?.message === 'string' && error.message.includes('provider is not enabled')) {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-3xl font-bold">QuizifyGenie</h1>
          </div>
        ...
        <CardContent>
          ...
          <Button 
            ...
            aria-disabled={loading || googleDisabled}
            title={googleDisabled ? 'Enable Google provider in Supabase to use this option' : undefined}
          >
            {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          ...
          {googleDisabled && (
            <p className="mt-2 text-xs text-muted-foreground text-center">
              Google sign-in is disabled. Enable it in Supabase &gt; Authentication &gt; Providers.
            </p>
          )}
